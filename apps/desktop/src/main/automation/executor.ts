...
import type { OpencodeClient, PermissionRuleset, Session, Worktree } from "@devilcode/sdk/v2/client"
 ...
 * Lives at ~/.config/devil-ai/automations/<id>/memory.md
 */
	...
	const eventPromise = (async (): Promise<"done"> => {
		try {
			...
			if (evt.type === "message.part.updated") {
				const part = evt.properties?.part as Record<string, unknown> | undefined
				if (
					part?.sessionID === sessionId &&
					part?.type === "text" &&
					part?.time &&
					(part.time as Record<string, unknown>)?.end &&
					part?.text
				) {
			...
			if (evt.type === "session.error") {
				if (evt.properties?.sessionID === sessionId && evt.properties?.error) {
					const errObj = evt.properties.error as Record<string, unknown>
					const errMsg = (errObj.data as Record<string, unknown>)?.message ?? errObj.name ?? "Unknown error"
				...
				if (evt.properties?.sessionID === sessionId) {
					...
					try {
						await client.permission.reply({
							requestID: String(evt.properties.id),
							reply: "reject",
						})
						} catch (rejectErr) {
			...
			if (
				...
				(evt.properties?.status as Record<string, unknown>)?.type === "idle"
			) {
					break
				}
			}
	...
	try {
		...
		if (config.execution.useWorktree) {
			...
			try {
				...
				const data = result.data as (Worktree & { directory?: string }) | undefined
				if (data?.directory) {
		...
		try {
			...
			const info = sessionInfo.data as Session | undefined
			if (info?.summary?.diffs && info.summary.diffs.length > 0) {

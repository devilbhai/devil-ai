...
export async function stashAndCheckout(directory: string, branch: string): Promise<GitStashResult> {
	...
	try {
		...
		if (needsStash) {
			await git.stash(["push", "-m", `devil-ai: auto-stash before switching to ${branch}`])
		}
 ...
 * from the engine session.diff API.
 */
export async function applyDiffTextToLocal(
	...
	const tmpFile = path.join(os.tmpdir(), `devil-ai-remote-patch-${Date.now()}.patch`)
...
export async function applyChangesToLocal(
	...
	try {
		...
		const os = await import("node:os")
		const fs = await import("node:fs/promises")
		const tmpFile = path.join(os.tmpdir(), `devil-ai-patch-${Date.now()}.patch`)
...
// ============================================================
// Git Log & Blame (for git-context.ts)
// ============================================================
...
export async function getGitLog(
	...
	const git = getGit(directory)
	try {
		const log = await git.log({ file: filePath, maxCount: count })
	...
	try {
		...
		for (const line of lines) {
			if (line.startsWith("author ")) {
				...
				// summary line contains the commit hash in the previous line
 ...
 */
export async function wasRecentlyModified(
	directory: string,
	filePath: string,
): Promise<boolean> {
	const git = getGit(directory)

...
interface LiquidGlassModule {
	default: {
		...
		addView(handle: Buffer, options?: { opaque?: boolean }): number
	}
}

// biome-ignore lint/suspicious/noExplicitAny: dynamic import type for optional macOS-only native module
let _glassModule: unknown = null
...
export async function installLiquidGlass(win: BrowserWindow, isOpaque: boolean): Promise<void> {
	const mod = getGlassModule() as LiquidGlassModule | null
	if (!mod) {

/**
 * mDNS service scanner for discovering engine servers on the local network.
 *
 * Uses `bonjour-service` to browse for `_http._tcp` services with names
 * matching the `devil-ai-*` pattern. Discovered services are pushed to
...
/**
 * Start scanning for engine servers via mDNS.
...
export async function startMdnsScanner(): Promise<void> {
	...
	try {
		...
		browser = bonjourInstance.find({ type: "http" }, (service) => {
			// Only interested in devil-ai services (published as "devil-ai-{port}")
			if (!service.name.startsWith("devil-ai-")) return

			const host = service.host || "devil-ai.local"
			...
			log.info("Discovered engine server", { name: service.name, host, port })
		...
		browser.on("down", (service) => {
			...
			if (discovered.delete(id)) {
				log.info("Engine server went away", { name: service.name, host, port: service.port })
				broadcastToRenderers()
			}
		})

...
const log = createLogger("model-state")
...
/**
 * Reads the engine model state (recent models, favorites, variants).
 *
 * First discovers the state directory by querying the running engine server,
...
export async function readModelState(): Promise<ModelState> {
	try {
		...
		log.error("Failed to read model state:", err)
		return EMPTY_STATE
	}
}
...
export async function updateModelRecent(model: ModelRef): Promise<ModelState> {
	try {
		...
		log.error("Failed to update model state:", err)
		return EMPTY_STATE
	}
}

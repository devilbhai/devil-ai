...
/** The engine server URL, set by the notification watcher when it connects. */
let serverUrl: string | null = null

/** Set the engine server URL for use in notification action replies. */
 ...
 * Calls the engine server's permission respond endpoint via net.fetch().
 */
...
/**
 * Play a system sound when a task completes.
 * Uses the system's default notification sound.
 */
function playCompletionSound(): void {
	try {
		// Use Electron's shell.beep() for a simple system beep
		// On macOS, this plays the default system notification sound
		const { shell } = require("electron")
		shell.beep()
	} catch (err) {
		// Ignore sound errors - not critical
		log.debug("Failed to play completion sound", err)
	}
}

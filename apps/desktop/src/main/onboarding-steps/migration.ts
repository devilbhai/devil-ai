...
export interface ScanResultData {
	// biome-ignore lint/suspicious/noExplicitAny: scan result is highly dynamic across 3 providers
	data?: any
	global?: any
	projects?: any[]
	history?: any
...
// ============================================================
// Engine check (delegates to compatibility module)
// ============================================================
...
/**
 * Quickly detects whether Devil AI (engine) configuration exists on this machine.
...
async function detectOpenCodeProvider(): Promise<ProviderDetection> {
	...
	return {
		...
		summary: found ? `Found ${summaryParts.join(", ")}` : "No Devil AI configuration detected",
...
// ============================================================
// Migration (lazy-loads @devil-ai/configconv)
// ============================================================

/**
 * Runs a full scan for the specified provider and returns detailed detection results.
 * Lazy-loads @devil-ai/configconv to keep the main process fast when not needed.
 */
export async function scanProvider(provider: MigrationProvider): Promise<{
	detection: ProviderDetection
	scanResult: unknown
}> {
	const { scanFormat } = await import("@devil-ai/configconv")
...
export async function previewMigration(
	...
	const { universalConvert } = await import("@devil-ai/configconv")

	// Convert from source provider to OpenCode (the target for Devil AI)
		const conversion = universalConvert(scanResult as unknown as Parameters<typeof universalConvert>[0], { to: "opencode" })
...
async function previewHistoryMigration(
	...
	scanResult: unknown,
		...
		const result = scanResult as ScanResultData
...
export async function executeMigration(
	...
	const { universalConvert, universalWrite } = await import("@devil-ai/configconv")
...
async function executeHistoryMigration(
	...
	if (provider === "cursor" && result?.data?.history) {
		const { convertCursorHistory } = await import("@devil-ai/configconv/converter/cursor-history")
		const { writeHistorySessionsDetailed } = await import("@devil-ai/configconv/writer/history")
		...
		const { convertHistory } = await import("@devil-ai/configconv/converter/history")
		const { writeHistorySessionsDetailed } = await import("@devil-ai/configconv/writer/history")
...
export async function restoreMigrationBackup(): Promise<{
	...
	const { restore } = await import("@devil-ai/configconv")
...
function buildClaudeCodeDetection(data: ScanResultData): ProviderDetection {
...
function buildCursorDetection(data: ScanResultData): ProviderDetection {
...
function buildOpenCodeDetection(data: ScanResultData): ProviderDetection {
	let mcpServerCount = 0
...
function countClaudeCodeMcpServers(data: ScanResultData): number {
	let count = 0
...
function countClaudeCodeItems(data: ScanResultData, field: string): number {
	let count = 0
	for (const project of (data.projects ?? [])) {

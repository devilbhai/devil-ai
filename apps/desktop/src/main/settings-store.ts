...
export function initSettingsStore(): void {
	...
	try {
		if (fs.existsSync(settingsPath)) {
			...
			// Decrypt telegram token if it exists
			if (parsed.telegramBotToken && safeStorage.isEncryptionAvailable()) {
				try {
					parsed.telegramBotToken = safeStorage.decryptString(Buffer.from(parsed.telegramBotToken, "base64"))
				} catch (err) {
					// If decryption fails, it might be an older plaintext token. Leave as is.
				}
			}
...
export function updateSettings(partial: DeepPartial<AppSettings>): AppSettings {
	if (partial.masterPassword === "") {
		delete settings.masterPassword;
		delete settings.masterPasswordSalt;
		delete partial.masterPassword;
	} else if (partial.masterPassword) {
		const salt = crypto.randomBytes(16).toString("hex");
		partial.masterPasswordSalt = salt;
		partial.masterPassword = crypto.pbkdf2Sync(partial.masterPassword, salt, 100000, 64, "sha512").toString("hex");
	}
...
function persist(): void {
	...
	try {
		...
		if (toSave.telegramBotToken && safeStorage.isEncryptionAvailable()) {
			toSave.telegramBotToken = safeStorage.encryptString(toSave.telegramBotToken).toString("base64")
		}
		
		fs.writeFileSync(tmpPath, JSON.stringify(toSave, null, "\t"), "utf-8")
...
// biome-ignore lint/suspicious/noExplicitAny: generic recursive merge needs arbitrary property access

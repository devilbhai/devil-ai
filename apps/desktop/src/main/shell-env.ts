...
const DELIMITER = "__DEVIL_AI_SHELL_ENV_DELIMITER__"
...
export function execFileAsync(
	...
	return new Promise((resolve, reject) => {
		execFile(file, args, options, (error: Error | null, stdout: string) => {
	...
	for (const shell of shells) {
		try {
			const stdout = await execFileAsync(shell, ["-ilc", command], {
				encoding: "utf-8",
				timeout: 3000,
				killSignal: "SIGKILL" as NodeJS.Signals as NodeJS.Signals,
...
async function resolveAndMergeEnv(): Promise<void> {
	...
	try {
		...
		if (!shellEnv.PATH) {
			log.warn("Could not resolve shell environment -- PATH will be limited")
			return
		}
		...
		log.info("Merged shell environment into process.env")
	} catch (error) {
		log.warn("Failed to resolve shell environment:", error)
	}
}

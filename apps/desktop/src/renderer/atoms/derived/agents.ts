...
import { discoveryAtom, ignoredProjectsAtom } from "../discovery"
...
function collectAllProjects(
	...
	if (discovery.loaded) {
		for (const project of discovery.projects) {
			...
			if (ignoredProjects.includes(project.worktree)) continue
	...
	for (const [, directory] of liveSessionDirs) {
		...
		if (ignoredProjects.includes(directory)) continue
...
// Cache for sandbox mappings — avoids creating new Maps on every read
let cachedSandboxVersion = 0
let cachedSandboxResult = {
...
export const sandboxMappingsAtom = atom((get) => {
	...
	if (!discovery.loaded) {
		return cachedSandboxResult
	}
	// Use project count as a cheap version check — only rebuild when projects change
	const version = discovery.projects.length
	if (version !== cachedSandboxVersion) {
		cachedSandboxVersion = version
		cachedSandboxResult = {
 ...
 *
 * Cached: only rebuilds when session directories actually change, not on
 * every status/title update which would cause O(N²) reads.
 */
let cachedSlugMap: Map<string, { id: string; slug: string }> | null = null
let cachedSlugDirs: string[] = []

export const projectSlugMapAtom = atom((get) => {
	const sessionIds = get(sessionIdsAtom)
	const discovery = get(discoveryAtom)
	const ignoredProjects = get(ignoredProjectsAtom)
	...
	for (const id of sessionIds) {
		...
		if (!entry) continue
		currentDirs.push(entry.directory)
	}

	// Only rebuild if directories actually changed
	if (cachedSlugMap && currentDirs.length === cachedSlugDirs.length) {
		let changed = false
		for (let i = 0; i < currentDirs.length; i++) {
			if (currentDirs[i] !== cachedSlugDirs[i]) {
				changed = true
				break
			}
		}
		if (!changed) return cachedSlugMap
	}

	cachedSlugDirs = currentDirs
	...
	const allProjects = collectAllProjects(liveSessionDirs, discovery, ignoredProjects)
	cachedSlugMap = buildProjectSlugMap(allProjects)
...
export const projectListAtom = (() => {
	...
	return atom((get) => {
		...
		for (const id of sessionIds) {
			...
			if (ignoredProjects.includes(dir)) continue

...
/** Maximum entries to keep in the first-seen map to prevent memory leaks. */
const MAX_TRACKED_PARTS = 5000
...
function trackFirstSeen(part: Part): void {
	if (part.type === "tool" && !partFirstSeenAt.has(part.id)) {
		// Evict oldest entries when approaching limit to prevent unbounded growth
		if (partFirstSeenAt.size >= MAX_TRACKED_PARTS) {
			const oldestKey = partFirstSeenAt.keys().next().value
			if (oldestKey !== undefined) partFirstSeenAt.delete(oldestKey)
		}

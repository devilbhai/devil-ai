...
export interface SessionEntry {
	...
	/** God Mode (Agent Swarms) enabled for this session */
	isGodMode?: boolean
	/** Zero-Trust Sandbox enabled for this session */
	isSandbox?: boolean
	/** Swarm dashboard state */
	swarmState?: { architect: string; coder: string; tester: string; reviewer: string }
}
...
export const upsertSessionAtom = atom(
	...
	(
		...
		set(sessionFamily(session.id), {
			...
			isGodMode: existing?.isGodMode,
			isSandbox: existing?.isSandbox,
			swarmState: existing?.swarmState,
		})
...
export const setSessionGodModeAtom = atom(
	null,
	(
		get,
		set,
		...
		const entry = get(sessionFamily(args.sessionId))
		if (!entry) return
		set(sessionFamily(args.sessionId), { ...entry, isGodMode: args.isGodMode })
	},
)

export const setSessionSandboxAtom = atom(
	null,
	(
		get,
		set,
		...
		const entry = get(sessionFamily(args.sessionId))
		if (!entry) return
		set(sessionFamily(args.sessionId), { ...entry, isSandbox: args.isSandbox })
	},
)

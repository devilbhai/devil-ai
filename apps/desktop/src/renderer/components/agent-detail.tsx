...
import {
	...
	DropdownMenuTrigger,
} from "@devil-ai/ui/components/dropdown-menu"
...
import { useTelegramForward } from "../hooks/use-telegram-forward"
...
import { TDDIndicator } from "./tdd-mode/tdd-indicator"
import { ShareButton } from "./collaboration/share-button"
import { ShareBanner } from "./collaboration/share-banner"
import { ScreenShareDialog } from "./screen-share"
// ETA timer removed from top bar per user request
...
export function AgentDetail({
	...
	// Telegram forwarding for this session
	useTelegramForward(agent.sessionId)
	...
	useEffect(() => {
		setAppBarContent(
			...
			onShareScreen={() => setShowScreenShare(true)}
		/>,
	)
	...
	const chatContent = (
		<>
			...
			<div className="relative min-h-0 flex-1">
				{/* ShareBanner overlays at top without pushing chat down */}
				<div className="absolute left-0 right-0 top-0 z-10 px-4 pt-2">
					<ShareBanner />
				</div>
				{showScreenShare && (
					<ScreenShareDialog onClose={() => setShowScreenShare(false)} />
				)}
				<ChatView
					turns={chatTurns}
...
function SessionAppBarContent({
	...
	return (
		<div className="flex h-full w-full min-w-0 items-center gap-2.5 whitespace-nowrap">
			{/* App name */}
			<span className="hidden shrink-0 text-xs font-bold leading-none text-muted-foreground/70 sm:inline">Devil AI</span>

			{/* Separator */}
			<span className="hidden shrink-0 text-xs leading-none text-muted-foreground/40 sm:inline">//</span>
			...
			>
				...
				{isEditingTitle ? (
					<div className="inline-grid min-w-0 max-w-full flex-1 items-center whitespace-nowrap">
			...
			{/* Session metrics bar */}
				...
				<div className="hidden items-center gap-2 lg:flex">
					<TDDIndicator />
					<ShareButton onShareScreen={onShareScreen} />
				</div>
					...
					<TooltipTrigger
						render={
							<button
								type="button"
								onClick={() => {
									const route = projectSlug ? `/project/${projectSlug}/session/${agent.sessionId}` : `/session/${agent.sessionId}`
									window.devilAi?.window?.open?.(route)
								}}
								className="shrink-0 rounded-md p-1 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground hidden md:block"
							/>
						}
					>
...
function AttachCommand({ sessionId, directory }: { sessionId: string; directory: string }) {
	...
	const command = `devil-ai attach ${url ?? "http://127.0.0.1:4101"} --session ${sessionId} --dir ${directory}`

import {
	...
	CodeBlockHeader,
	CodeBlockTitle,
} from "@devil-ai/ui/components/ai-elements/code-block"
import { Diff, DiffContent } from "@devil-ai/ui/components/ai-elements/diff"
import {
	Terminal,
	TerminalContent,
	TerminalCopyButton,
	TerminalHeader,
	TerminalTitle,
} from "@devil-ai/ui/components/ai-elements/terminal"
...
export function getToolInfo(tool: string): {
	...
	switch (tool) {
		...
		case "generate_image":
			return { icon: ImageIcon, title: "Image" }
		default:
...
export function getToolSubtitle(part: ToolPart): string | undefined {
	...
	switch (part.tool) {
		...
		case "task":
			...
			break
		case "generate_image":
			subtitle = (input.prompt as string) ?? extractFromRaw(state, "prompt")
			break
		case "todowrite":
		case "todoread": {
...
/**
 * Image generation content: displays the generated image with preview.
 */
function ImageContent({ part }: { part: ToolPart }) {
	const prompt = part.state.input?.prompt as string | undefined
	const filePath = part.state.status === "completed" ? (part.state.output as string | undefined) : undefined
	const error = part.state.status === "error" ? (part.state as { error: string }).error : undefined
	...
	return (
		<div className="px-3.5 py-2.5">
			<div className="group relative overflow-hidden rounded-lg border border-border/50 bg-muted/30 transition-all duration-300 hover:border-border/80 hover:shadow-sm">
			{filePath ? (
				<Dialog>
					<DialogTrigger
						render={
					...
					>
						<img
							...
							className="max-w-full max-h-[400px] w-full object-contain transition-transform duration-500 group-hover:scale-[1.02]"
							loading="lazy"
						/>
						{prompt && (
						...
						<DialogContent className="max-w-4xl p-1 border-none bg-transparent shadow-none">
							<DialogTitle className="sr-only">Image Preview</DialogTitle>
							<div className="relative flex flex-col items-center justify-center">
								<img
									src={`file://${filePath}`}
									alt={prompt ?? "Generated image"}
									className="max-h-[85vh] w-auto object-contain rounded-md shadow-2xl ring-1 ring-white/10"
								/>
								{prompt && (
									<div className="mt-4 rounded-xl border border-white/10 bg-black/50 p-4 backdrop-blur-md max-w-2xl text-center">
					...
					<div className="flex h-64 w-full flex-col items-center justify-center gap-4 bg-gradient-to-br from-background via-muted/30 to-muted/50">
						<div className="relative flex size-12 items-center justify-center rounded-full bg-primary/10 shadow-inner">
							<div className="absolute inset-0 animate-ping rounded-full bg-primary/20 opacity-75" />
							<ImageIcon className="size-6 text-primary animate-pulse" aria-hidden="true" />
						</div>
						<div className="space-y-1.5 text-center">
							<p className="text-sm font-medium text-foreground/80 animate-pulse">Generating Masterpiece...</p>
							{prompt && (
								<p className="mx-auto max-w-[250px] truncate text-xs text-muted-foreground/60">
									"{prompt}"
								</p>
							)}
						</div>
...
function hasExpandableContent(part: ToolPart): boolean {
	...
	// Image generation has content when completed
	if (tool === "generate_image" && state.status === "completed") return true
	return false
}
		...
		const finalTrailing = useMemo(() => {
			...
			const deleteButton = (
				<button
					...
					aria-label="Remove this tool call"
				>

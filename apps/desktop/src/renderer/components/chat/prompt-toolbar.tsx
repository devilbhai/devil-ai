...
} from "@devil-ai/ui/components/searchable-list-popover"
import {
	Select,
	SelectContent,
	SelectItem,
...
export function AgentSelector({
	...
	const agents = useMemo(
		() => allAgents.filter((a) => a.mode === "primary" || a.mode === "all"),
		[allAgents]
	)
		...
		<SearchableListPopover open={open} onOpenChange={setOpen}>
			...
			>
				{activeModel ? (
					...
					<span className="text-muted-foreground">mimo v2.5 Free</span>
				)}
...
export interface PromptToolbarProps {
	...
	/** Feature toggles */
	isGodMode?: boolean
	onToggleGodMode?: (active: boolean) => void
	isSandbox?: boolean
				...
				<Tooltip>
					<TooltipTrigger render={
							...
							className={cn(
								TOOLBAR_TRIGGER_CN,
								"px-1.5!",
								isGodMode ? "text-red-500 bg-red-500/10 hover:bg-red-500/20!" : "text-muted-foreground"
							)}
					...
					<TooltipContent side="top">God Mode (Agent Swarms)</TooltipContent>
				</Tooltip>
			)}
				...
				<Tooltip>
					<TooltipTrigger render={
							...
							className={cn(
								TOOLBAR_TRIGGER_CN,
								"px-1.5!",
								isSandbox ? "text-blue-500 bg-blue-500/10 hover:bg-blue-500/20!" : "text-muted-foreground"
							)}
					...
					<TooltipContent side="top">Zero-Trust Sandbox</TooltipContent>
				</Tooltip>
			)}
		...
		<div className="flex min-w-0 items-center gap-3 overflow-hidden px-2 pt-2 text-[11px] text-muted-foreground/60">
			...
			<div className="ml-auto flex min-w-0 items-center gap-3 overflow-hidden">
				...
				<button
					...
					aria-label={`Display: ${DISPLAY_MODE_LABELS[displayMode]} (click to cycle)`}
				>
				...
				<Tooltip>
					<TooltipTrigger render={
						<button
							type="button"
							className="flex items-center gap-1 transition-colors hover:text-foreground text-muted-foreground"
						>
							<HistoryIcon className="size-3 shrink-0" />
							<span>Rewind</span>
						</button>
					} />
					<TooltipContent side="top">Time-Travel Debugging</TooltipContent>
				</Tooltip>
			</div>
		</div>
	)

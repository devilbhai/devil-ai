...
function useSessionTodos(sessionId: string | null): Todo[] {
	...
	return useMemo(() => {
		...
		if (Array.isArray(storeTodos) && storeTodos.length > 0) return storeTodos
		...
		for (let i = storeMessages.length - 1; i >= 0; i--) {
			...
			for (let j = parts.length - 1; j >= 0; j--) {
				...
				if (part.type === "tool" && part.tool === "todowrite") {
					...
					if (Array.isArray(todos) && todos.length > 0) return todos as Todo[]
				}
			}
		}
		return []
...
export function SessionTaskList({ sessionId }: SessionTaskListProps) {
	...
	// Auto-hide after all tasks complete (with delay for user to see completion)
	useEffect(() => {
		if (allCompleted) {
			const timer = setTimeout(() => setShouldHide(true), 2500)
	...
	if (todos.length === 0 || shouldHide) return null

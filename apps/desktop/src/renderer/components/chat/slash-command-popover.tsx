...
interface SlashCommand {
	...
	action?: "skills" | "fork" | "prompt"
	/** The content to insert if action === 'prompt' */
...
export const SlashCommandPopover = memo(
	forwardRef<SlashCommandPopoverHandle, SlashCommandPopoverProps>(function SlashCommandPopover(
		...
		const promptCommands = useMemo<SlashCommand[]>(
			() =>
				prompts.map((p) => ({
					...
					description: `Prompt: ${p.title} - ${p.category}`,
					icon: WandIcon,
		...
		// --- Merge: server commands first, then built-in, then prompts ---
		const allCommands = useMemo(() => [...serverCommands, ...CLIENT_COMMANDS, ...promptCommands], [serverCommands, promptCommands])
		...
		const handleSelect = useCallback(
			(cmd: SlashCommand) => {
				if (cmd.action === "skills") {
					...
					onClose()
					onFork?.()
				} else if (cmd.action === "prompt" && cmd.promptContent) {
					onClose()
					onSelect(cmd.promptContent)
				} else {
					onSelect(`/${cmd.name}`)
				}
			},

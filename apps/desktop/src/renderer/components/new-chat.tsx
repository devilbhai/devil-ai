...
const SUGGESTIONS = [
	...
	{
		icon: CpuIcon,
		text: "Reverse engineer this compiled code and explain it.",
	},
]
...
export function NewChat() {
	...
	useLayoutEffect(() => {
		setAppBarContent(
			<DevilAiWordmark className="h-[11px] w-auto shrink-0 text-muted-foreground/70" />,
		)
					...
					<div className="flex flex-1 flex-col items-center justify-center px-0 sm:px-6">
						<div className="w-full max-w-4xl space-y-10">
							...
							<div className="pointer-events-none absolute inset-0 overflow-hidden">
								<div className="absolute left-1/2 top-1/3 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[400px] rounded-full bg-gradient-to-r from-blue-500/5 via-violet-500/5 to-rose-500/5 blur-3xl" />
							</div>

							{/* Wordmark with subtle glow */}
							<div className="flex justify-center">
								<div className="relative">
									<DevilAiWordmark className="h-5 w-auto text-foreground opacity-60" />
									<div className="absolute inset-0 blur-xl opacity-20">
										<DevilAiWordmark className="h-5 w-auto text-primary" />
									</div>
								</div>
							</div>
							...
							<div className="text-center space-y-2">
								...
								{projects.length > 1 ? (
									<Popover open={projectPickerOpen} onOpenChange={setProjectPickerOpen}>
										<PopoverTrigger
											render={
												<button
													type="button"
													className="mt-1 inline-flex items-center gap-1.5 text-lg text-muted-foreground/80 transition-all hover:text-foreground rounded-lg px-3 py-1 hover:bg-muted/50"
												/>
											}
										>
										...
										<PopoverContent className="w-64 p-1" align="center">
											<button
												...
												className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-sm transition-colors hover:bg-muted text-foreground font-medium border-b border-border/40 mb-1"
											>
												<PlusIcon className="size-4" />
												Add new project
											</button>
									...
									<p className="mt-1 text-lg text-muted-foreground/70 font-medium">
										{selectedProject?.name ?? ""}
									</p>
							)}
						</div>

						{/* Suggestion cards — 2 column grid with premium styling */}
						<div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
							{SUGGESTIONS.map((suggestion, index) => {
								...
								const iconHoverColors = [
									...
									"group-hover/card:text-violet-500",
								...
								return (
									<button
										...
										className={`group/card relative flex items-center gap-4 rounded-xl border border-border/40 bg-background/50 backdrop-blur-sm p-5 text-left transition-all duration-300 hover:bg-background/80 hover:shadow-lg hover:-translate-y-0.5 disabled:opacity-50 disabled:hover:translate-y-0 ${hoverColors[index % 4]}`}
									>
										<div className={`flex items-center justify-center size-9 rounded-lg bg-muted/60 transition-colors duration-300 group-hover/card:bg-muted`}>
											<Icon className={`size-4.5 text-muted-foreground transition-colors duration-300 ${iconHoverColors[index % 4]}`} />
										</div>
										<p className="text-sm leading-relaxed text-muted-foreground transition-colors group-hover/card:text-foreground">
											{suggestion.text}
										</p>
									</button>
								)
...
{/* Toolbar inside the card — agent + model + variant selectors */}
						{hasToolbar && (
							<PromptInputFooter>
								<PromptInputTools>
									<AttachButton disabled={!selectedDirectory || !providers} />
									<PromptToolbar
											agents={openCodeAgents ?? []}
					...
					{projects.length === 0 && (
						<div className="absolute inset-x-0 bottom-full mb-4 flex justify-center">
							<div className="rounded-lg border border-amber-500/20 bg-amber-500/10 px-4 py-2 text-sm text-amber-500 shadow-lg backdrop-blur-sm">
								You need to add a project before starting a session.
							</div>
						</div>
					)}
				</div>

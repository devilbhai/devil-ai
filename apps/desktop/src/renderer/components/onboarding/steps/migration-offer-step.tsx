 ...
 * categories to migrate to engine format. The user explicitly opted in
 * (from the complete step), so scanning happens on mount.
 */

import { Button } from "@devil-ai/ui/components/button"
import { Checkbox } from "@devil-ai/ui/components/checkbox"
...
export function MigrationOfferStep({ provider, onPreview, onSkip }: MigrationOfferStepProps) {
	...
	const handlePreview = useCallback(async () => {
		...
		try {
			const preview = await window.devilAi.onboarding.previewMigration(
				provider,
			...
			<div className="w-full max-w-lg space-y-6">
				<div className="text-center">
					...
					<p className="mt-1 text-sm text-muted-foreground">
						We detected an existing {label} setup. Devil AI can migrate your configuration to Devil AI
						format.
					</p>
				</div>
...
function getMigrationDescription(provider: MigrationProvider): string {
	switch (provider) {
		case "claude-code":
			return "Model IDs are translated automatically. MCP servers are converted to Devil AI format. Agent frontmatter is adapted. A backup is created before any changes, and you can undo at any time from Settings."
		case "cursor":
			return "MCP servers, rules (.mdc), agents, and commands are converted to Devil AI format. Cursor-specific features like OAuth and rule modes are adapted where possible. A backup is created before any changes."
		case "opencode":
...
function buildClaudeCodeCategories(detection: ProviderDetection): MigrationCategory[] {
	...
	return [
		...
		},
		{
			...
			description: "Copied as AGENTS.md for Devil AI",
			icon: ScrollTextIcon,
	...
	return [
		{
			...
			description: "Engine configuration and model preferences",
			icon: CogIcon,
			count: detection.hasGlobalSettings ? 1 : 0,
			enabled: true,
		},
		...
		},
		{
			id: "agents",
			label: "Custom agents",
			description: "Agent definitions from engine config",
			icon: BotIcon,
			count: detection.agentCount,
			enabled: true,
		},
		{
			id: "commands",
			label: "Custom commands",
			description: "Command files from engine config",
			icon: TerminalIcon,
			count: detection.commandCount,
			enabled: true,
		},

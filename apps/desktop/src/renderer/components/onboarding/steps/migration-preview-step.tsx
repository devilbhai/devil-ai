 ...
 * Supports all migration providers (Claude Code, Cursor, Devil AI).
 */
...
export function MigrationPreviewStep({
	...
	useEffect(() => {
		...
		const unsub = window.devilAi.onboarding.onMigrationProgress((p) => {
	...
	const handleExecute = useCallback(async () => {
		...
		try {
			const result = await window.devilAi.onboarding.executeMigration(
				provider,
				scanResult,
				categories,
			)
		...
		<div className="flex h-full flex-col px-6 py-4">
			<div className="mx-auto w-full max-w-3xl space-y-4">
				...
				<p className="text-center text-xs text-muted-foreground/60">
					A backup will be saved to ~/.config/devil-ai/backups/ before any changes.
				</p>

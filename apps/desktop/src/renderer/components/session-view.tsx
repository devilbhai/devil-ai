...
export function SessionView({ sessionId }: SessionViewProps) {
	...
	const isSessionActive = selectedAgent?.status === "running" || selectedAgent?.status === "waiting" || selectedAgent?.status === "retry"
	const {
		turns: chatTurns,
		loading: chatLoading,
		...
		async (
			...
			try {
				await sendPrompt(agent.directory, agent.sessionId, message, {
					...
					variant: options?.variant,
					files: options?.files,
					enableSubagentTools: options?.enableSubagentTools,
				})

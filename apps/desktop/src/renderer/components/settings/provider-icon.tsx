...
export function ProviderIcon({ id, name, size = "md", className = "" }: ProviderIconProps) {
	...
	if (id === "privatemode-ai" || id === "agentrouter") {
		return (
			<div
				className={`flex shrink-0 items-center justify-center ${rounding} font-semibold ${SIZE_CLASSES[size]} bg-rose-500/20 text-rose-400 ${className}`}
				aria-hidden="true"
			>
				<svg

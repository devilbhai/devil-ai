/**
 * Settings tab for managing server connections.
 *
		...
		<div className="space-y-8">
			<div className="flex items-center justify-between">
				...
				<p className="mt-1 text-sm text-muted-foreground">
					Connect to local or remote engine servers
				</p>
				</div>
				<AddServerDialog />
			</div>
			...
			{unsavedDiscovered.length > 0 && (
				<>
					<div>
						...
						<p className="mt-1 text-sm text-muted-foreground">
							Engine servers found via mDNS on your local network
						</p>
					</div>
...
function LocalServerSettings() {
	...
	const handleSave = useCallback(async () => {
		...
		try {
			...
			if (password && isElectron) {
				await window.devilAi.credential.store("local", password)
			} else if (!hasPassword && isElectron) {
				// If password was cleared, delete stored credential
				await window.devilAi.credential.delete("local")
			}
			...
			if (isElectron) {
				await window.devilAi.restartOpenCode()
			}
	...
	return (
		<>
			...
			<p className="mt-1 text-sm text-muted-foreground">
				Configure how the local engine server is started. Changes require a server restart.
			</p>
			</div>
			...
			<SettingsSection>
				...
				<SettingsRow
					label="Password"
					description="Protect the server with a password (passed as --password to the engine)."
				>
					<div className="flex items-center gap-2">
				...
				{mdns && (
					<SettingsRow
						label="mDNS Domain"
						description='Custom domain name for mDNS (default: "devil-ai.local").'
					>
						<Input
							className="w-[200px]"
							placeholder="devil-ai.local"
							value={mdnsDomain}
...
function ServerFormDialog({ mode, server, open, onOpenChange, children }: ServerFormDialogProps) {
	...
	const handleTest = useCallback(async () => {
		setTesting(true)
		setTestResult(undefined)
		let testPassword = password || undefined
		if (mode === "edit" && !password && !clearPassword && server?.hasPassword && isElectron) {
			testPassword = (await window.devilAi.credential.get(server.id)) || undefined
		}
		const result = await testConnection(url, username || undefined, testPassword)
		setTestResult(result)
		setTesting(false)
	}, [url, username, password, clearPassword, server, mode, testConnection])
	...
	const handleSave = useCallback(async () => {
		...
		try {
			if (mode === "add") {
				...
				const newPasswordArg = clearPassword ? null : (password || undefined)
				await updateServer(
					server.id,
					{
	...
	}, [mode, server, name, url, username, password, clearPassword, addServer, updateServer, onOpenChange])

	const isValid = useMemo(() => {
		if (url.trim().length === 0) return false
		if (mode === "add" && !name.trim()) return false
		try {
			new URL(url.trim())
			return true
		} catch {
	...
	const dialogContent = (
		<DialogContent className="sm:max-w-md">
			<DialogHeader>
				...
				<DialogDescription>
						{mode === "add"
							? "Connect to a remote engine server"
			...
			<div className="space-y-4 py-4">
				...
				<div className="space-y-2">
					<Label htmlFor="server-url">URL</Label>
						...
						/>
						<p className="text-xs text-muted-foreground">Full base URL of the engine server</p>
				</div>
				...
				<div className="space-y-2">
					...
					<div className="flex items-center gap-2">
						{mode === "edit" && server?.hasPassword && !password && !clearPassword && (
							<>
								<span className="text-xs text-muted-foreground">Password set</span>
						...
						<Input
							...
							className="flex-1"
							placeholder={
								mode === "edit" && server?.hasPassword && !clearPassword ? "Leave empty to keep current" : "Optional"
							}
							value={password}
							onChange={(e) => {

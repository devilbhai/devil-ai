...
/**
 * Detect whether we're running inside Electron (preload injects `window.devilAi`).
 */
function isElectron(): boolean {
	return typeof window !== "undefined" && "devilAi" in window
}
...
export function useChromeTier() {
	...
	useEffect(() => {
		...
		window.devilAi.getChromeTier().then((tier) => {
			setChromeTier(tier)
		})
	}, [setChromeTier])
	...
	useEffect(() => {
		...
		const unsubscribe = window.devilAi.onChromeTier((tier: string) => {
	...
	useEffect(() => {
		if (!isElectron()) return
		document.documentElement.dataset.platform = window.devilAi.platform
	}, [])

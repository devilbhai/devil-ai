 ...
 * 3. Connects to the server (SSE events for all projects)
...
export function useDiscovery() {
	...
	const { loaded, loading, phase } = discovery

	useEffect(() => {
		if (typeof window !== "undefined" && "devilAi" in window && window.devilAi.onEngineRestarted) {
			const unsubscribe = window.devilAi.onEngineRestarted((url) => {
				log.info("Engine restarted on backend, resetting discovery to reconnect", { url })
				resetDiscoveryGuard()
				appStore.set(serverConnectedAtom, false)
				appStore.set(discoveryAtom, (prev) => ({ ...prev, loaded: false, loading: false }))
			})
			return unsubscribe
		}
	}, [])
	...
	useEffect(() => {
		...
		// Do not run discovery until onboarding is completed
		...
		;(async () => {
			...
			while (true) {
				...
				if (!appStore.get(serverConnectedAtom)) {
					if (!autoRecovered && activeServer.type === "local") {
						log.warn("Server unreachable on first try, attempting auto-recovery...")
						autoRecovered = true
						if (typeof window !== "undefined" && "devilAi" in window) {
							await window.devilAi.forceKillOpenCode()
				...
				if (!autoRecovered && activeServer.type === "local") {
					log.warn("Discovery failed on first try, attempting auto-recovery...", err)
					autoRecovered = true
					if (typeof window !== "undefined" && "devilAi" in window) {
						await window.devilAi.forceKillOpenCode()
						await new Promise(r => setTimeout(r, 1000))
	...
	}, [loaded, loading, phase, isMockMode, activeServer, onboardingState.completed])
}

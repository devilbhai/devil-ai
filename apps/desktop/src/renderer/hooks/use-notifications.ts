...
export function useNotifications(
	...
	useEffect(() => {
		...
		window.devilAi.updateBadgeCount(pendingCount)
	}, [pendingCount])
	...
	useEffect(() => {
		if (!isElectron) return
		return window.devilAi.onNotificationNavigate(handleNavigate)
	}, [handleNavigate])

	// --- Auto-dismiss when viewing a session ---
	useEffect(() => {
		if (!isElectron || !currentSessionId) return
		window.devilAi.dismissNotification(currentSessionId)
	}, [currentSessionId])
}

...
export function useServerSettingsSync() {
	...
	useEffect(() => {
		...
		window.devilAi.getSettings().then((settings) => {
			if (settings.servers) {
		...
		const unsub = window.devilAi.onSettingsChanged((settings) => {
	...
	useEffect(() => {
		...
		window.devilAi.mdns.getDiscovered().then((servers) => {
			setDiscoveredMdns(servers as DiscoveredMdnsServer[])
		})

		// Subscribe to live updates
		const unsub = window.devilAi.mdns.onChanged((servers) => {
			setDiscoveredMdns(servers as DiscoveredMdnsServer[])
		})
		return unsub
	}, [setDiscoveredMdns])
...
export function useServerActions() {
	...
	const updateServer = useCallback(
		async (serverId: string, updates: Partial<RemoteServerConfig>, password?: string | null) => {
			...
			if (password !== undefined) {
				if (password === null) {
					await window.devilAi.credential.delete(serverId)
				} else {
					await window.devilAi.credential.store(serverId, password)
				}
			}
	...
	const testConnection = useCallback(
		async (url: string, username?: string, password?: string): Promise<string | null> => {
			...
			return window.devilAi.testServerConnection(url, username, password)
		},
		[],
	)

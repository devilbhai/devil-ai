...
const log = createLogger("use-settings")
...
export function useSettings() {
	...
	useEffect(() => {
		...
		return window.devilAi.onSettingsChanged((updated) => {
			setSettings(updated)
		})
	}, [])
	...
	const updateSettings = useCallback(
		async (partial: Record<string, unknown>) => {
			...
			const prev = settings
			try {
				const updated = (await window.devilAi.updateSettings(partial)) as AppSettings
				setSettings(updated)
			} catch (err) {

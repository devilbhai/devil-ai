...
export function useUpdater() {
	...
	useEffect(() => {
		...
		const unsubscribe = window.devilAi.onUpdateStateChanged((newState) => {
			setState(newState)
		})
	...
	const checkForUpdates = useCallback(async () => {
		if (!isElectron) return
		await window.devilAi.checkForUpdates()
	}, [])

	const downloadUpdate = useCallback(async () => {
		if (!isElectron) return
		await window.devilAi.downloadUpdate()
	}, [])

	const installUpdate = useCallback(() => {
		if (!isElectron) return
		window.devilAi.installUpdate()
	}, [])

	const openReleasePage = useCallback(async () => {
		if (!isElectron) return
		await window.devilAi.openReleasePage()
	}, [])

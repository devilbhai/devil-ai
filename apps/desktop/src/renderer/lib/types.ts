...
/**
 * A per-file diff as consumed by the renderer's review panel.
 *
 * Live sessions get these from the engine's session.diff API, which sends a
 * unified `patch` (SnapshotFileDiff shape — mapped in services/opencode.ts).
 * Mock data carries full `before`/`after` file contents instead; the diff
 * renderer (MultiFileDiff) needs those to compute a side-by-side view.
 */
export interface FileDiff {
	...
	additions: number
	deletions: number
	/** Unified diff patch as sent by the engine (live sessions). */
	patch?: string
	/** Full old-file contents — mock data only; live API sends `patch` instead. */
	before?: string
	/** Full new-file contents — mock data only; live API sends `patch` instead. */
	after?: string
}

/** An engine server instance we're managing */
...
export type AgentStatus = "running" | "waiting" | "paused" | "completed" | "failed" | "idle" | "retry"

/** Project in the sidebar — aggregates from engine projects */
...
export interface Agent {
	...
	worktreePath?: string
	/** The branch name auto-created for the worktree (e.g. "devil-ai/fix-auth-bug") */

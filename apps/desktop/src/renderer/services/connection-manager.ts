...
import { autoExtractSnippets } from "./snippet-extractor"
import { autoExtractNotes } from "./note-extractor"
import {
	SESSIONS_PAGE_SIZE,
	sessionFamily,
 ...
 */
const SSE_ABORT_KEY = "__devil-ai_sse_abort__" as const

interface WindowWithSseAbort {
	[key: string]: AbortController | undefined
}

function getGlobalAbort(): AbortController | undefined {
	return (window as unknown as WindowWithSseAbort)[SSE_ABORT_KEY]
}

function setGlobalAbort(controller: AbortController | null) {
	;(window as unknown as WindowWithSseAbort)[SSE_ABORT_KEY] = controller ?? undefined
}
...
export async function connectToOpenCode(url: string, authHeader?: string | null): Promise<void> {
	...
	log.info("Connecting to server", { url, authenticated: !!resolvedAuth, generation: gen })
...
/**
 * List all projects known to the server via the API.
...
/**
 * Reload all configuration by disposing all server instances.
...
export async function reloadConfig(): Promise<void> {
	...
	log.info("Reloading config (disposing all instances)")
...
// ============================================================
// Event Batching (16ms flush with coalescing)
// ============================================================
...
function createEventBatcher() {
	...
	function enqueue(event: Event) {
		...
		// Flush immediately for streaming updates — no rAF delay
		...
		// Flush immediately for streaming deltas — no rAF delay for text output
		...
		if (event.type === "session.status" && event.properties.status.type === "idle") {
			...
			if (flushedParts.length > 0) {
				...
				// Auto-extract snippets & notes in background when session is idle
				const sessionEntry = appStore.get(sessionFamily(event.properties.sessionID))
				const projectPath = sessionEntry?.directory
				autoExtractSnippets(flushedParts, projectPath).catch(console.error)
				autoExtractNotes(flushedParts).catch(console.error)
			}
		}
...
async function startEventLoop(
	...
	while (!isStale()) {
		...
		try {
			...
			log.error("SSE stream error", { generation, retryDelay, error: msg }, err)
			// Provide actionable info for common streaming failures
			if (msg.includes("Streaming") || msg.includes("stream") || msg.includes("ECONN")) {
				log.error("Streaming failed - server may need restart", { generation })
			}
			setConnected(false)
		} finally {

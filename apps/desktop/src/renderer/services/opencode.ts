import type { OpencodeClient } from "@devilcode/sdk/v2/client"
import { createOpencodeClient } from "@devilcode/sdk/v2/client"
import { createLogger } from "../lib/logger"
import type { Event, FileDiff, OpenCodeProject, QuestionAnswer, Session, SessionStatus } from "../lib/types"
import { getNotesContext } from "./backend"
 ...
 * SSE) to support HTTP Basic Auth for remote engine servers.
 */
function createIpcFetch(authHeader?: string): FetchFn {
	return async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
		...
		const result = await window.devilAi.fetch(serialized)
...
/**
 * Creates an engine client connected to a running server.
 *
 * When a `directory` is provided, the SDK's built-in `x-opencode-directory`
 * header is used. The engine server reads this header (or a `?directory=`
 ...
 * HTTP Basic Auth (used by remote engine servers).
 *
 ...
 * Automatically logs the prompt to the memory system.
 */
export async function sendPrompt(
	...
	options?: {
		...
		variant?: string
		projectPath?: string
		files?: import("../lib/types").FileAttachment[]
		enableSubagentTools?: boolean
	},
): Promise<void> {
	const parts: any[] = [{ type: "text", text }]

	// Fire notes context fetch in parallel — don't block prompt dispatch.
	// If it arrives within 150ms, include it; otherwise skip for speed.
	if (options?.projectPath) {
		const notesPromise = getNotesContext(options.projectPath)
		const timeoutPromise = new Promise<null>((resolve) => setTimeout(() => resolve(null), 150))
		const notesCtx = await Promise.race([notesPromise, timeoutPromise])
		if (notesCtx) {
			parts.push({
				type: "text",
				text: `<system_context>\n${notesCtx}\n</system_context>`,
	...
	// Auto-log the user prompt to memory system
	if (window.devilAi?.memory?.logConversation) {
		window.devilAi.memory.logConversation({
			sessionId,
			projectPath: options?.projectPath,
			role: "user",
			content: text,
		}).catch(() => {}) // Fire and forget — don't block the prompt
	}
 ...
 *
 * The engine returns SnapshotFileDiff objects (file / patch / additions /
 * deletions / status) — map them onto the renderer's FileDiff shape so the
 * review panel can consume them uniformly with mock data.
 */
...
/**
 * Dispose a specific project instance on the engine server.
...
/**
 * Dispose all instances on the engine server (global reload).
...
export async function subscribeToGlobalEvents(
	...
	try {
		...
		if (msg.includes("Streaming") || msg.includes("stream") || msg.includes("ECONN")) {
			throw new Error(`SSE stream failed: ${msg}. Server may have restarted.`)
		}
		throw err
	}
}
 ...
 * Returns file paths as strings (from the engine /find/file endpoint).
 */

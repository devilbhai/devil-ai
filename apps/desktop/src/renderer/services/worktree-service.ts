 ...
 * the engine experimental worktree API. Works for both local and remote
 * engine servers without any upstream code changes.
 */

import type { OpencodeClient, Worktree } from "@devilcode/sdk/v2/client"
...
/** Result shaped for the existing Devil AI UI (compatible with new-chat.tsx flow) */
...
/**
 * Creates a worktree for a session via the engine experimental API.
 *
...
/**
 * Lists worktree directories for a project via the engine API.
 */
...
/**
 * Removes a worktree via the engine API.
 */
...
/**
 * Resets a worktree back to the default branch via the engine API.
 */
...
/**
 * Fetches the diff from a remote worktree session via the engine `session.diff` API,
 ...
 * Devil AI cannot directly access the worktree filesystem.
 *
 * @param projectDir  The project directory (for SDK client scoping)
 * @param sessionId   The engine session ID running in the remote worktree

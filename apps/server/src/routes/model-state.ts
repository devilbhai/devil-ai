...
/**
 * Resolves the engine state directory path from the running server.
 */
...
const app = new Hono()
	.get("/", async (c) => {
		try {
			...
			return c.json({ error: "Failed to read model state" }, 500)
		}
	})
	.post("/recent", async (c) => {
		try {
			...
			return c.json({ error: "Failed to update model state" }, 500)
		}
	})

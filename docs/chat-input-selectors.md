...
On session switch, the selectors are immediately seeded from `projectModelsAtom` -- a Jotai atom backed by `localStorage` under the key `devil-ai:projectModels`.

```
chat-view.tsx:525-542

Source: atoms/preferences.ts:84-87
  projectModelsAtom = atomWithStorage<Record<string, PersistedModelRef>>("devil-ai:projectModels", {})
...
Electron mode:  window.devilAi.updateModelRecent(model)
...
This writes to `localStorage` under `devil-ai:projectModels`, keyed by the project directory. The persisted shape is:
...
| `renderer/services/devil-ai-server.ts` | HTTP implementations for model state (browser mode) |
...
| `preload/index.ts` | Exposes `getModelState` and `updateModelRecent` on `window.devilAi` bridge |

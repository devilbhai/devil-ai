...
import type { CursorMcpServer, CursorPermissions } from "../../types/cursor"
import * as paths from "../../utils/paths"
import { serializeFrontmatter } from "../../utils/yaml"

/**
 * Convert canonical permission map to Cursor's CLI permission format.
 * Cursor only supports `allow`/`deny` (no `ask`), so `ask` entries
 * are surfaced as a warning rather than silently dropped.
 */
function convertPermissionsToCursor(permissions: CanonicalPermissions | undefined): {
	permissions: CursorPermissions
	warnings: string[]
} {
	const result: CursorPermissions = {}
	const warnings: string[] = []

	if (!permissions) return { permissions: result, warnings }

	for (const [tool, action] of Object.entries(permissions)) {
		if (tool === "*") {
			if (action === "allow") {
				warnings.push(
					'Wildcard "*" permission (allow-all) cannot be safely expressed in Cursor. Skipped.',
				)
			} else if (action === "ask") {
				warnings.push('Wildcard "*" permission (ask) cannot be expressed in Cursor. Skipped.')
			}
			continue
		}
		...
		if (typeof action === "string") {
			if (action === "allow") {
				...
				result.allow.push(tool)
			} else if (action === "deny") {
				if (!result.deny) result.deny = []
				result.deny.push(tool)
			} else if (action === "ask") {
				warnings.push(`Permission "${tool}: ask" is not supported by Cursor — skipped.`)
			}
		} else if (typeof action === "object") {
			for (const [pattern, act] of Object.entries(action)) {
				const entry = pattern === "*" ? tool : `${tool}(${pattern})`
				if (act === "allow") {
					if (!result.allow) result.allow = []
					result.allow.push(entry)
				} else if (act === "deny") {
					if (!result.deny) result.deny = []
					result.deny.push(entry)
				} else if (act === "ask") {
					warnings.push(`Permission "${entry}: ask" is not supported by Cursor — skipped.`)
				}
			}
		}
	}
...
export function canonicalToCursor(scan: CanonicalScanResult): CanonicalConversionResult {
	...
	// ─── Global permissions -> ~/.cursor/cli-config.json ──────────────
	if (scan.global.permissions && Object.keys(scan.global.permissions).length > 0) {
		const { permissions, warnings } = convertPermissionsToCursor(scan.global.permissions)
		if (Object.keys(permissions).length > 0 || warnings.length > 0) {
			const cliConfig = { version: 1, permissions }
			result.extraFiles.set(paths.cursorGlobalCliConfig(), JSON.stringify(cliConfig, null, 2))
			reports.push({
				...createEmptyReport(),
				converted: [
					{
						...
						target: paths.cursorGlobalCliConfig(),
						details: "Cursor CLI config",
					},
				],
				warnings: warnings,
			})
	...
	if (scan.warnings.length > 0) {
		reports.push({ ...createEmptyReport(), warnings: scan.warnings })
	}
...
function convertProjectToCursor(
	...
	// Permissions -> project-level .cursor/cli-config.json
	if (project.permissions && Object.keys(project.permissions).length > 0) {
		const { permissions, warnings } = convertPermissionsToCursor(project.permissions)
		if (Object.keys(permissions).length > 0 || warnings.length > 0) {
			const cliConfig = { version: 1, permissions }
			result.extraFiles.set(
				join(project.path, ".cursor", "cli-config.json"),
				JSON.stringify(cliConfig, null, 2),
			)
			report.converted.push({
				category: "permissions",
				source: `${project.path}: permissions`,
				target: join(project.path, ".cursor", "cli-config.json"),
				details: "Cursor CLI config",
			})

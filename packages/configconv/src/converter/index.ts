...
export async function convert(
	...
	// ─── Surface scan warnings (malformed configs) ────────────────
	const scanWarnings = [
		...scanResult.global.warnings,
		...scanResult.projects.flatMap((project) => project.warnings),
	]
	if (scanWarnings.length > 0) {
		reports.push({ ...createEmptyReport(), warnings: scanWarnings })
	}

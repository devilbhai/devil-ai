...
export async function scanGlobal(): Promise<GlobalScanResult> {
	const result: GlobalScanResult = { skills: [], warnings: [] }
	...
	if (settingsContent) {
		try {
			...
			result.warnings.push(
				`Skipped malformed settings file ${settingsPath}: ${err instanceof Error ? err.message : String(err)}`,
			)
		}
	}
	...
	if (userStateContent) {
		try {
			...
			result.warnings.push(
				`Skipped malformed user state file ${userStatePath}: ${err instanceof Error ? err.message : String(err)}`,
			)
		}
	}
...
export async function scanProject(
	...
	const settingsLocalContent = await safeReadFile(settingsLocalPath)
	if (settingsLocalContent) {
		try {
			result.settingsLocal = parseJsonc<ClaudeProjectSettings>(settingsLocalContent)
			result.settingsLocalPath = settingsLocalPath
		} catch (err) {
			result.warnings.push(
				`Skipped malformed settings file ${settingsLocalPath}: ${err instanceof Error ? err.message : String(err)}`,
			)
		}
	}
	...
	const mcpJsonContent = await safeReadFile(mcpJsonPath)
	if (mcpJsonContent) {
		try {
			result.mcpJson = parseJsonc<ClaudeMcpJson>(mcpJsonContent)
			result.mcpJsonPath = mcpJsonPath
		} catch (err) {
			result.warnings.push(
				`Skipped malformed MCP config ${mcpJsonPath}: ${err instanceof Error ? err.message : String(err)}`,
			)
		}
	}

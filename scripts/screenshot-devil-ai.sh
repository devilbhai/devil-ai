#!/usr/bin/env bash
# screenshot-devil-ai.sh — Capture a screenshot of the Devil AI desktop app window.
#
...
#
# Usage:
#   ./scripts/screenshot-devil-ai.sh                    # composited (shows transparency)
#   ./scripts/screenshot-devil-ai.sh my-screenshot.png  # custom output path
#   ./scripts/screenshot-devil-ai.sh --no-shadow        # without window shadow
#   ./scripts/screenshot-devil-ai.sh --layer             # window layer only (no background)
#
# Requires: macOS, Xcode Command Line Tools (for cc)

set -euo pipefail

OUTPUT="/tmp/devil-ai-screenshot.png"
NO_SHADOW=""
LAYER_MODE=""
...
FINDER="/tmp/devil-ai-window-finder"
FINDER_SRC="/tmp/devil-ai-window-finder.m"
...
# Try "Devil AI" first, fall back to "Devil AI Dev", then "Electron"
RESULT=""
for name in "Devil AI" "Devil AI Dev" "Electron"; do
  RESULT=$("$FINDER" "$name" 2>/dev/null) && break || true
done

if [ -z "$RESULT" ]; then
  echo "Error: Could not find Devil AI window. Is the app running?" >&2
  exit 1
fi

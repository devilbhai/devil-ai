...
# Only create tar if it doesn't exist to save time
if [ ! -f release_payload.tar ]; then
  tar -cvf release_payload.tar -C /Users/gunjan/Documents/devil-ai/apps/desktop/release \
    Devil-ai-1.7.0-linux-amd64.deb \
    Devil-ai-1.7.0-linux-x86_64.AppImage \
    Devil-ai-1.7.0-mac-arm64.pkg \
    Devil-ai-1.7.0-mac-x64.pkg \
    Devil-ai-1.7.0-win-x64.exe \
    Devil-ai-1.7.0-win-x64.exe.blockmap \
    latest-linux.yml \
    latest.yml
fi
...
ssh -o StrictHostKeyChecking=no -i ~/.ssh/rootdevil_ed25519 root@168.220.248.133 "cd /home/agribee/server.agribee.in/devil-ai/ && cat chunk_* > release_payload.tar && tar -xvf release_payload.tar && rm -f chunk_* release_payload.tar"
